...
int ReUpdate(void) {
	...
	// 2019-10-06 Workaround is to set the lap limite with respect to how many timestep are required, then
	// and le the Python API restart the env instead. This will take a little bit more time than if we stopped it
	// from here directly, BUT apparently, the latter is not possible, as it hangs up on Wainting for request on %PORT%
	...
	// if( getRecordHuman() && ( getRecTimestepLimit() > 0 && timestep >= getRecTimestepLimit())) {
		...
		// Atempting to force restart by directly changing the car properties
		// ReInfo->s->cars[0]->RESTART=1;
		// ReSortCars();
	// }
	...
	elapsed_ms = duration_cast< milliseconds >( system_clock::now().time_since_epoch()).count() - last_ms_checkpoint;
	// printf( "### DEBUG: Elpased ms %ld\n", elapsed_ms);

	if( elapsed_ms >= 1000) {
		printf( "### DEBUG Freq: Timestep per sec: %d\n", freq_timestep);
		last_ms_checkpoint = duration_cast< milliseconds >( system_clock::now().time_since_epoch()).count();
		elapsed_ms = 0;
		freq_timestep = 0;
	}
